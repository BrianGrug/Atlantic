    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        if (event.getAction() == Action.RIGHT_CLICK_BLOCK) {
            ...
            if (state instanceof Skull) {
                ...
                player.sendMessage(ChatColor.AQUA + "This is " + ChatColor.YELLOW
                        + (skull.getSkullType() == SkullType.PLAYER && skull.hasOwner() ? skull.getOwner() : "a " + WordUtils.capitalizeFully(skull.getSkullType().name()) + " skull") + ChatColor.AQUA
                        + '.');
            }
        }
    }
