package me.scifi.hcf.pvpclass.bard;
...
public class BardClass extends PvpClass implements Listener {
    ...
    public BardClass(HCF plugin) {
        ...
        this.bardEffects.put(Material.SUGAR, new BardEffect(45, new PotionEffect(PotionEffectType.SPEED, 120, 2), new PotionEffect(PotionEffectType.SPEED, HELD_EFFECT_DURATION_TICKS, 1)));
        this.bardEffects.put(Material.BLAZE_POWDER, new BardEffect(50, new PotionEffect(PotionEffectType.INCREASE_DAMAGE, 120, 0), new PotionEffect(PotionEffectType.INCREASE_DAMAGE,
                HELD_EFFECT_DURATION_TICKS, 0)));
        this.bardEffects.put(Material.IRON_INGOT, new BardEffect(35, new PotionEffect(PotionEffectType.DAMAGE_RESISTANCE, 80, 2), new PotionEffect(PotionEffectType.DAMAGE_RESISTANCE,
                HELD_EFFECT_DURATION_TICKS, 0)));
        this.bardEffects.put(Material.GHAST_TEAR, new BardEffect(30, new PotionEffect(PotionEffectType.REGENERATION, 60, 2), new PotionEffect(PotionEffectType.REGENERATION,
                HELD_EFFECT_DURATION_TICKS, 0)));
        this.bardEffects.put(Material.FEATHER, new BardEffect(40, new PotionEffect(PotionEffectType.JUMP, 120, 2), new PotionEffect(PotionEffectType.JUMP, HELD_EFFECT_DURATION_TICKS, 1)));
    }
        ...
        if (action == Action.RIGHT_CLICK_AIR || (!event.isCancelled() && action == Action.RIGHT_CLICK_BLOCK)) {
            ...
            if (bardData != null) {
                ...
                player.sendMessage(ChatColor.GOLD + "You have just used " + name + " buff " + ChatColor.RED + Lang.fromPotionEffectType(bardEffect.clickable.getType()) + ' '
                        ...
                        + "Your energy is now " + ChatColor.GREEN + ((newEnergy * 10.0) / 10.0) + ChatColor.YELLOW + '.');
            }
        }
    }
