...
public class SubclaimWandListener implements Listener {
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (event.isCancelled())
            return;
        /*
         * if ((action == Action.LEFT_CLICK_BLOCK || action == Action.LEFT_CLICK_AIR) && player.isSneaking()) { //TODO: Possibly open an Anvil to make a name for the Subclaim? }
         */
        ...
        if (action == Action.LEFT_CLICK_BLOCK || action == Action.RIGHT_CLICK_BLOCK) {
            ...
            if (plugin.getClaimHandler().canSubclaimHere(player, blockLocation)) {
                ...
                player.sendMessage(ChatColor.GREEN + "Set the location of subclaim selection " + ChatColor.YELLOW + selectionId + ChatColor.GREEN + " to: " + ChatColor.GOLD + '(' + ChatColor.YELLOW
